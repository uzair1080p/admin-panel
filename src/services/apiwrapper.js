const BASE_URL = 'https://api-dev.ratetracker.io';
...
class APIWrapper {
    ...
    static async listUsers() {
      return await this._makeRequest('listTransactions', 'get', `/users`, {});
    }
      ...
      try {
        ...
        if(response.status >= 200 && response.status < 300){
          if(response.headers.has('x-access-token')){
            localStorage.setItem("tokens", JSON.stringify(response.headers.get('x-access-token')));
          }
          return data.data;
        } else {
